...
// x:    SIZE = 784
// W1:   SIZE x H1 = 784 x 256
// b1:   H1 = 256
// h1:   H1 = 256

// W2:   H1 x H2 = 256 x 128
// b2:   H2 = 128
// h2:   H2 = 128

// W3:   H2 x CLASSES = 128 x 10
// b3:   CLASSES = 10
// out:  CLASSES = 10
...
void train_model(MODEL* model){
    ...
    CUDA_CHECK(cudaMalloc(&d_train_data, NUM_TRAIN * SIZE * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_train_labels, NUM_TRAIN * CLASSES * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_W1, SIZE * H1 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_b1, H1 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_h1, H1 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_W2, H1 * H2 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_b2, H2 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_h2, H2 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_W3, H2 * CLASSES * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_b3, CLASSES * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_out, CLASSES * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_delta3, CLASSES * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_delta2, H2 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_delta1, H1 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_loss, sizeof(float)));

    CUDA_CHECK(cudaMemcpy(
        d_train_data,
        train_data,
        NUM_TRAIN * SIZE * sizeof(float),
	...
	d_train_labels,
	train_label,
	NUM_TRAIN * CLASSES * sizeof(float),
	cudaMemcpyHostToDevice
    ));
    ...
    for (int epoch=0; epoch<EPOCHS; epoch++) {
	    ...
	    // ---------- Forward ----------
	    const float *d_x = d_train_data + n * SIZE;

            launch_forward_relu(d_x, d_W1, d_b1, d_h1, SIZE, H1);
	    launch_forward_relu(d_h1, d_W2, d_b2, d_h2, H1, H2);
    
	    launch_forward_linear(d_h2, d_W3, d_b3, d_out, H2, CLASSES);
	    launch_softmax(d_out, CLASSES);
   
            // ---------- Loss ----------
	    const float *d_label = d_train_labels + n * CLASSES;

            launch_loss(d_out, d_label, d_loss, CLASSES);

	    // ---------- Backprop ----------
	    launch_output_delta(d_out, d_label, d_delta3, CLASSES);
	    launch_hidden_delta(d_delta3, d_W3, d_h2, d_delta2, H2, CLASSES);
	    launch_hidden_delta(d_delta2, d_W2, d_h1, d_delta1, H1, H2);

            // ---------- Update ----------
       	    launch_update_weights(d_W3, d_h2, d_delta3, LR, H2, CLASSES);
	    launch_update_bias(d_b3, d_delta3, LR, CLASSES);
	    launch_update_weights(d_W2, d_h1, d_delta2, LR, H1, H2);
	    launch_update_bias(d_b2, d_delta2, LR, H2);
	    launch_update_weights(d_W1, d_x, d_delta1, LR, SIZE, H1);
	    launch_update_bias(d_b1, d_delta1, LR, H1);
	...
	printf("Epoch %d loss: %f\n", epoch, loss_cpu / NUM_TRAIN);
    }
    
    CUDA_CHECK(cudaMemcpy(model->W1, d_W1, SIZE * H1 * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(model->b1, d_b1, H1 * sizeof(float), cudaMemcpyDeviceToHost));

    CUDA_CHECK(cudaMemcpy(model->W2, d_W2, H1 * H2 * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(model->b2, d_b2, H2 * sizeof(float), cudaMemcpyDeviceToHost));

    CUDA_CHECK(cudaMemcpy(model->W3, d_W3, H2 * CLASSES * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(model->b3, d_b3, CLASSES * sizeof(float), cudaMemcpyDeviceToHost));
